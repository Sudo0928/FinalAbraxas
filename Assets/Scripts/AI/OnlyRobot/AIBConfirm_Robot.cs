...
public class AIBConfirm_Robot : AIBehaviorBase
{
        ...
        if (Vector3.Distance(transform.position, soundPos) < SoundDis)
        {
            ...
            switch (decivel)   // 들은 소리가
            {
                ...
                case 2:                    // 발 소리라면
                    ...
                    if (Time.time - _WatchStartTime > 0.5f)
                    {
                        bIsListened = true;
    ...
    public override void Run()
    {
        ...
        SoundGauge = 0;
        //sight._WatchDist = 3.0f;
        bIsListened = false;
        Agent.isStopped = true;
        Agent.velocity = Vector3.zero;
        _WatchStartTime = Time.time;
        view.viewAngle = 360.0f;
